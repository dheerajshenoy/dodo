...
LEKTRA can be configured using \textbf{TOML} or \textbf{Lua}. Both files are optional and both can be present; the TOML file is applied first, and the Lua file (\pathtt{\$HOME/.config/lektra/init.lua}) is sourced afterwards so it can override any TOML setting or add runtime behaviour.

Let us look at how to configure using TOML first. The configuration file should be at \pathtt{\$HOME/.config/lektra/config.toml}. Run \keys{:open\_config} to open whichever config file(s) exist — a chooser appears if both are present.
...
Place your Lua init file at \pathtt{\$HOME/.config/lektra/init.lua}. It is sourced at startup after the TOML config is applied.
